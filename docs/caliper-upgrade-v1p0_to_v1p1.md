...
### NavigationEvent property changes
...
### DigitalResource property changes
...
### MediaLocation property changes
...
## Caliper JSON-LD document "thinning"
Caliper 1.0 `Event` JSON-LD documents exhibited the following characteristics:

* Every Caliper `Entity` JSON object described in a Caliper JSON-LD `Event` was required to include a JSON-LD `@context` property with a value that duplicated the enclosing `Event` context.  This approach failed to leverage JSON-LD Context inheritance rules.
* All `Event` and `Entity` types expressed as JSON-LD were required to reference all properties in the JSON-LD document, irrespective of disposition (i.e., required or optional) or the actual value assignment, if any.
* Terms 
